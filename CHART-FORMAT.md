...
**Status:** living specification. **The listening audio is Copyist's own as of 2026-09-20**, and as of 2026-09-21 THE BAND IS REAL: the listen renders on recorded instruments — a sample shelf of openly licensed libraries (the Salamander grand, Karoryfer basses and saxes, Virtuosity and Swirly drums, VSCO2 CE brass, winds and strings with real staccato takes, a General MIDI SoundFont as the floor and the plain synth as the no-samples fallback), played by Copyist's own pure-stdlib SFZ and SF2 engines. Articulations perform: staccato, tenuto, accent, marcato, ghosts, slurred legato, scoops, plops, falls and doits (bends, the way a player makes them), hairpins swell through held notes, fermatas hold time itself, and dynamics choose recorded velocity layers, level and brightness together. **Slash bars realize in the listening document** (Matthew's ruling, 2026-09-20 with Rocco): drums play time, bass walks, comping instruments comp from the chart's own chord symbols — deterministic, the pages keep their slashes, findings name every realized part. The chairs humanize (nobody plays on the grid, sections sit cents and milliseconds apart) and a small room sits behind the band with per-family sends. And as of 2026-09-20 SO ARE THE PART PAGES: prototype/chartengrave.py draws every single-staff part as a PDF from pure stdlib — staves, clefs, keys, meters, beams, ties, slurs, tuplets, articulations, ghosts, slashes, kicks, cues, lyrics with melisma lines, chord symbols, dynamics, texts, rehearsal boxes, repeats, voltas, multirests with counts. The conductor score engraves too — every part stacked and synchronized at score size — so a chart with no grand-staff part builds with NO MuseScore anywhere, and the build says so out loud. The grand staff engraves too (brace, per-staff clefs and signatures, voices stemmed apart, connected barlines), so as of tonight EVERY chart in the book — Subway Psalm, Greenline, Sunday Crossing, and Matt's Blues with its seventeen lifted parts — builds with no MuseScore at all. The one documented decline: grace notes in lifted engravings, refused in a sentence. As of 2026-09-20 the pages are set in a real music font: Leland (MuseScore's OFL-licensed SMuFL face, in fonts/) is embedded in every PDF Copyist draws — clefs, accidentals, noteheads, rests, flags, time signatures, dynamics and articulations are font glyphs, with the hand-drawn art kept as the no-font fallback. Optical spacing landed 2026-09-20: onsets take the rhythm's share of the bar but never sit closer than their ink allows, and beamed groups agree on one stem direction. Cross-bar ties landed 2026-09-20: a tie or slur over a barline is one true arc, and one broken at a system turn draws its outgoing half to the edge and an incoming half to the landing note. Text faces landed 2026-09-20 — each look embeds its own hand (Edwin default/engraved, MuseJazz Text for jazz, Petaluma Script for handwritten), and the alto clef joined the pages. The beauty bench is clear. The same night's correctness sweep: accidentals follow the key and the bar (in-key notes print bare, cancels print the natural, ties restate nothing), and a transposing part's chord symbols print written, not concert — a Bb horn's F7 reads G7. Designed 2026-08-30 against the measured
...
- Chord spelling: root with `b` or `#`, then quality: `7 9 11 13 maj maj7
  ...
  6 69 add9 madd9 7b5 7#5 7b9 7#9 7#11 13#11 13sus 7#9b13 7#9#11 7b13 13b9 alt`, plus slash
